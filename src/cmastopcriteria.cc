...
namespace libcmaes
{
    ...
    :_active(true)
  {
    ...
    StopCriteriaFunc<TGenoPheno> noProgress = [](const CMAParameters<TGenoPheno> &cmap, const CMASolutions &cmas)
	  ...
	  if (maxSteps <= 0) 
	  	return CONT;
	  int bestCandidates = cmas._best_seen_candidates_hist.size();
	  if (bestCandidates < maxSteps) 
	  	return CONT;

	  double newBestScoreEver = cmas._best_seen_candidates_hist.back().get_fvalue();
	  double oldBestScoreEver = cmas._best_seen_candidates_hist[bestCandidates - maxSteps].get_fvalue();

	  if (newBestScoreEver >= oldBestScoreEver) {
		LOG_IF(INFO,!cmap._quiet) << "stopping criteria NoProgress " << 
                "newBestScore: " << newBestScoreEver << ", oldBestScore: " << oldBestScoreEver << "\n";
		return NOPROGRESS;
	  }
